#-------------------------------------------------------------------------------
# Looking at the targets for opportunities
#-------------------------------------------------------------------------------
...
df <- separate_rows(df, target_type, sep=",")
df$target_type <- trimws(df$target_type)
df <- df %>% filter(!target_type == "NA")


# Draw the plot ----------------------------------------------------------------

print(
ggplot(df, aes(fct_rev(fct_infreq(target_type)))) + 
...
# Add labels
labs(title="Opportunities by target type",
...
df <- df %>% group_by(target_type, mobility_type) %>% count(name="n")
df <- df %>% group_by(target_type) %>% mutate(per = 100 / sum(n) * n)
