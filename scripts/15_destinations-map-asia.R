#-------------------------------------------------------------------------------
# Mapping no. calls by destination - Asia
#-------------------------------------------------------------------------------
...
v <- un_geo %>% filter(region == "Asia") %>% select(country)
...
# Rename South Korea
df <- df %>% mutate_all(funs(str_replace_all(., "South Korea", 
                                             "Republic of Korea"))) 
...
map2 <- st_crop(world_map %>% filter(name_long %in% v2), xmin = 20, xmax = 150, ymin = -20, ymax = 50)

# Set bounding box for the map and data
map <- st_crop(world_map, xmin = 20, xmax = 150, ymin = -20, ymax = 55)
