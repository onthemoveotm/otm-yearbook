#-------------------------------------------------------------------------------
# Mapping no. calls by destination - Africa
#-------------------------------------------------------------------------------
...
v <- un_geo %>% filter(region == "Africa") %>% select(country)
...
# Merge the centroids into the counts & filter out the NAs
...
map2 <- st_crop(world_map %>% filter(name_long %in% v2),
                xmin = -30, xmax = 50, ymin = -45, ymax = 45)

# Set bounding box for the map and data
map <- st_crop(world_map, xmin = -55, xmax = 75, ymin = -45, ymax = 45)
